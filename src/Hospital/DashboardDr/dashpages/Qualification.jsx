...
const Qualification = () => {
  ...
  useEffect(() => {
    ...
    onValue(ref(database, `Photos/Hospital/${id}/1`), (snapshot) => {
      if (snapshot.exists()) {
        setgetUrl1(snapshot.val().url1);
      } else {
        // Handle the case when the data doesn't exist
        console.error(`Data for Photos/${id}/1 does not exist.`);
      }
    });

    onValue(ref(database, `Photos/Hospital/${id}/2`), (snapshot) => {
      if (snapshot.exists()) {
        setgetUrl2(snapshot.val().url2);
      } else {
        // Handle the case when the data doesn't exist
        console.error(`Data for Photos/${id}/2 does not exist.`);
      }
    });

    onValue(ref(database, `Photos/Hospital/${id}/3`), (snapshot) => {
      if (snapshot.exists()) {
        setgetUrl3(snapshot.val().url3);
      } else {
        // Handle the case when the data doesn't exist
        console.error(`Data for Photos/${id}/3 does not exist.`);
      }
    });

    onValue(ref(database, `Photos/Hospital/${id}/4`), (snapshot) => {
      if (snapshot.exists()) {
        setgetUrl4(snapshot.val().url4);
      } else {
        // Handle the case when the data doesn't exist
        console.error(`Data for Photos/${id}/4 does not exist.`);
      }
    });

    onValue(ref(database, `Photos/Hospital/${id}/5`), (snapshot) => {
      if (snapshot.exists()) {
        setgetUrl5(snapshot.val().url5);
      } else {
        // Handle the case when the data doesn't exist
        console.error(`Data for Photos/${id}/5 does not exist.`);
      }
    });

    onValue(ref(database, `Photos/Hospital/${id}/6`), (snapshot) => {
      if (snapshot.exists()) {
        setgetUrl6(snapshot.val().url6);
      } else {
        // Handle the case when the data doesn't exist
        console.error(`Data for Photos/${id}/6 does not exist.`);
      }
    });

    onValue(ref(database, `Photos/Hospital/${id}/description`), (snapshot) => {
      if (snapshot.exists()) {
        setDescriptionTxt(snapshot.val().description);
      } else {
        // Handle the case when the data doesn't exist
        console.error(`Data for Photos/${id}/description does not exist.`);
      }
    });
  ...
  const postListRef = ref(database, `Photos/Hospital/${id}`);
  ...
  function uploadData() {
    const imgRef = ref_storage(storage, `Photos/Hospital/${id}/Photos/1`);
    ...
    uploadTask
      .then((snapshot) => {
        ...
        getDownloadURL(snapshot.ref).then((url) => {
          ...
          putData(`Photos/Hospital/${id}/1`, { url1: url })
  ...
  function uploadData2() {
    const imgRef = ref_storage(storage, `Photos/Hospital/${id}/Photos/2`);
    const uploadTask = uploadBytesResumable(imgRef, img);
    //Handle the promise returned by uploadBytesResumable
    uploadTask
      .then((snapshot) => {
        ...
        getDownloadURL(snapshot.ref).then((url) => {
          ...
          alert("Data uploaded succesfully, Please refresh the page")
          putData(`Photos/Hospital/${id}/2`, { url2: url })
        });
      })
      .catch((error) => {
        console.error('Error uploading image:', error);
  ...
  function uploadData3() {
    const imgRef = ref_storage(storage, `Photos/Hospital/${id}/Photos/3`);
    const uploadTask = uploadBytesResumable(imgRef, img);
    //Handle the promise returned by uploadBytesResumable
    uploadTask
      .then((snapshot) => {
        ...
        getDownloadURL(snapshot.ref).then((url) => {
          ...
          console.log('Download URL:', url1);
          alert("Data uploaded succesfully, Please refresh the page")
          putData(`Photos/Hospital/${id}/3`, { url3: url })
        });
      })
      .catch((error) => {
        console.error('Error uploading image:', error);
  ...
  function uploadData4() {
    const imgRef = ref_storage(storage, `Photos/Hospital/${id}/Photos/4`);
    const uploadTask = uploadBytesResumable(imgRef, img);
    //Handle the promise returned by uploadBytesResumable
    uploadTask
      .then((snapshot) => {
        ...
        getDownloadURL(snapshot.ref).then((url) => {
          ...
          console.log('Download URL:', url1);
          alert("Data uploaded succesfully, Please refresh the page")
          putData(`Photos/Hospital/${id}/4`, { url4: url })
        });
      })
      .catch((error) => {
        console.error('Error uploading image:', error);
  ...
  function uploadData5() {
    const imgRef = ref_storage(storage, `Photos/Hospital/${id}/Photos/5`);
    const uploadTask = uploadBytesResumable(imgRef, img);
    //Handle the promise returned by uploadBytesResumable
    uploadTask
      .then((snapshot) => {
        ...
        getDownloadURL(snapshot.ref).then((url) => {
          ...
          console.log('Download URL:', url1);
          alert("Data uploaded succesfully, Please refresh the page")
          putData(`Photos/Hospital/${id}/5`, { url5: url })
        });
      })
      .catch((error) => {
        console.error('Error uploading image:', error);
  ...
  function uploadData6() {
    const imgRef = ref_storage(storage, `Photos/Hospital/${id}/Photos/6`);
    const uploadTask = uploadBytesResumable(imgRef, img);
    //Handle the promise returned by uploadBytesResumable
    uploadTask
      .then((snapshot) => {
        ...
        getDownloadURL(snapshot.ref).then((url) => {
          ...
          console.log('Download URL:', url1);
          alert("Data uploaded succesfully, Please refresh the page")
          putData(`Photos/Hospital/${id}/6`, { url6: url })
        });
      })
      .catch((error) => {
        console.error('Error uploading image:', error);
  ...
  const uploadText = () => {
    ...
    putData(`Photos/Hospital/${id}/description`, { description: description })
  }
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        <div>
          <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
            <DrawerHeader />
            <h2 >Kindly Upload Your Hospital Photos</h2>
